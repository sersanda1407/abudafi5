        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <form action="" method="POST" id="formLaporan" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        <label class="form-label">Foto Dokumentasi Stok</label>
                        <small class="text-muted d-block">Upload foto kopi, teh, dan milo untuk dokumentasi:</small>
                        
                        <div class="row mt-2">
                            <div class="col-md-4">
                                <div class="card">
                                    <div class="card-body text-center">
                                        <h6 class="card-title">Kopi</h6>
                                        <div class="mb-2">
                                            <?php if ($edit_mode && !empty($laporan_edit['foto_kopi'])): ?>
                                                <img src="uploads/<?php echo $laporan_edit['foto_kopi']; ?>" class="img-thumbnail mb-2" style="max-height: 100px;">
                                                <br>
                                        ...
                                        <input type="file" class="form-control form-control-sm" name="foto_kopi" accept="image/*" 
                                            <?php echo (!$edit_mode && $shift_pagi_sudah_ada && $shift_sore_sudah_ada) ? 'disabled' : ''; ?>>
                            ...
                            <div class="col-md-4">
                                <div class="card">
                                    <div class="card-body text-center">
                                        <h6 class="card-title">Teh</h6>
                                        <div class="mb-2">
                                            <?php if ($edit_mode && !empty($laporan_edit['foto_teh'])): ?>
                                                <img src="uploads/<?php echo $laporan_edit['foto_teh']; ?>" class="img-thumbnail mb-2" style="max-height: 100px;">
                                                <br>
                                                <small class="text-muted">Foto saat ini</small>
                                            <?php else: ?>
                                        ...
                                        <input type="file" class="form-control form-control-sm" name="foto_teh" accept="image/*"
                                            <?php echo (!$edit_mode && $shift_pagi_sudah_ada && $shift_sore_sudah_ada) ? 'disabled' : ''; ?>>
                                        <small class="text-muted">Format: JPG, PNG (max 2MB)</small>
                                    </div>
                                </div>
                            ...
                            <div class="col-md-4">
                                <div class="card">
                                    <div class="card-body text-center">
                                        <h6 class="card-title">Milo</h6>
                                        <div class="mb-2">
                                            <?php if ($edit_mode && !empty($laporan_edit['foto_milo'])): ?>
                                                <img src="uploads/<?php echo $laporan_edit['foto_milo']; ?>" class="img-thumbnail mb-2" style="max-height: 100px;">
                                                <br>
                                                <small class="text-muted">Foto saat ini</small>
                                            <?php else: ?>
                                                <i class="fas fa-coffee fa-2x text-muted mb-2"></i>
                                        ...
                                        <input type="file" class="form-control form-control-sm" name="foto_milo" accept="image/*"
                                            <?php echo (!$edit_mode && $shift_pagi_sudah_ada && $shift_sore_sudah_ada) ? 'disabled' : ''; ?>>
                                        <small class="text-muted">Format: JPG, PNG (max 2MB)</small>
                                    </div>
                                </div>
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            ...
                            while ($row = mysqli_fetch_assoc($result)) {
                                ...
                                echo '<td>';
                                // Tampilkan icon foto jika ada
                                if (!empty($row['foto_kopi']) || !empty($row['foto_teh']) || !empty($row['foto_milo'])) {
                                    echo '<i class="fas fa-camera text-success" title="Ada foto dokumentasi"></i>';
                                } else {
                                    echo '<i class="fas fa-camera text-muted" title="Tidak ada foto"></i>';
                                }
                                echo '</td>';
                                echo '<td>';
        ...
        if (file) {
            ...
            reader.onload = function(e) {
                if (imgPreview) {
                    ...
                    newImg.className = 'img-thumbnail mb-2';
                    ...
                    card.querySelector('.mb-2').prepend(newImg);
                }
            }
...
function uploadFoto($file, $nama_file) {
    $target_dir = "uploads/";
    ...
    $file_name = $nama_file . '_' . date('Ymd_His') . '.' . $file_extension;
...
if (isset($_POST['simpan_laporan'])) {
    ...
    if (mysqli_num_rows($result_cek) > 0) {
        ...
        if (!empty($_FILES['foto_kopi']['name'])) {
            $upload_result = uploadFoto($_FILES['foto_kopi'], 'kopi');
            if (isset($upload_result['success'])) {
                $foto_kopi = $upload_result['success'];
            } else {
                echo '<script>alert("Error foto kopi: ' . $upload_result['error'] . '");</script>';
            }
        }
        
        if (!empty($_FILES['foto_teh']['name'])) {
            $upload_result = uploadFoto($_FILES['foto_teh'], 'teh');
            if (isset($upload_result['success'])) {
                $foto_teh = $upload_result['success'];
            } else {
                echo '<script>alert("Error foto teh: ' . $upload_result['error'] . '");</script>';
            }
        }
        
        if (!empty($_FILES['foto_milo']['name'])) {
            $upload_result = uploadFoto($_FILES['foto_milo'], 'milo');
            if (isset($upload_result['success'])) {
                $foto_milo = $upload_result['success'];
            } else {
                echo '<script>alert("Error foto milo: ' . $upload_result['error'] . '");</script>';
            }
        }
        
        // Simpan laporan harian
        $query = "INSERT INTO laporan_harian (tanggal, shift, pemasukan, pengeluaran, total, status, foto_kopi, foto_teh, foto_milo) 
                  VALUES ('$tanggal', '$shift', $pemasukan, $total_pengeluaran, $total, '$status', '$foto_kopi', '$foto_teh', '$foto_milo')";
...
if (isset($_POST['update_laporan'])) {
    ...
    if (mysqli_num_rows($result_cek) > 0) {
        ...
        // Proses upload foto
        $foto_kopi = $laporan_edit['foto_kopi'];
        $foto_teh = $laporan_edit['foto_teh'];
        $foto_milo = $laporan_edit['foto_milo'];
        
        if (!empty($_FILES['foto_kopi']['name'])) {
            $upload_result = uploadFoto($_FILES['foto_kopi'], 'kopi');
            if (isset($upload_result['success'])) {
                // Hapus foto lama jika ada
                if (!empty($foto_kopi) && file_exists("uploads/" . $foto_kopi)) {
                    unlink("uploads/" . $foto_kopi);
                }
                $foto_kopi = $upload_result['success'];
            } else {
                echo '<script>alert("Error foto kopi: ' . $upload_result['error'] . '");</script>';
        ...
        if (!empty($_FILES['foto_teh']['name'])) {
            ...
            if (isset($upload_result['success'])) {
                // Hapus foto lama jika ada
                if (!empty($foto_teh) && file_exists("uploads/" . $foto_teh)) {
                    unlink("uploads/" . $foto_teh);
                }
                $foto_teh = $upload_result['success'];
            } else {
                echo '<script>alert("Error foto teh: ' . $upload_result['error'] . '");</script>';
        ...
        if (!empty($_FILES['foto_milo']['name'])) {
            ...
            if (isset($upload_result['success'])) {
                // Hapus foto lama jika ada
                if (!empty($foto_milo) && file_exists("uploads/" . $foto_milo)) {
                    unlink("uploads/" . $foto_milo);
                }
                $foto_milo = $upload_result['success'];
            } else {
                echo '<script>alert("Error foto milo: ' . $upload_result['error'] . '");</script>';
